...
export default function PlayerPage() {
  ...
  // Filtra i giocatori basandosi sul termine di ricerca
  const filteredPlayers = players ? players.filter((player) => player.name.toLowerCase().includes(searchTerm.toLowerCase())) : [];
    ...
    return (
      <div className="h-full bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 text-lg">Caricamento giocatori...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="bg-gray-50 py-8 h-full">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-6">
          {/* <h1 className="text-4xl font-bold text-gray-900 mb-4">Giocatori</h1> */}
          <p className="text-xl text-gray-600 mb-4">Scopri tutti i giocatori disponibili</p>
        ...
        {players && players.length > 0 ? (
          filteredPlayers.length > 0 ? (
            <div className="space-y-3 h-[400px] overflow-y-auto">
              {filteredPlayers.map((player) => (
                <div key={player.id} className="bg-white rounded-lg shadow-sm hover:shadow-md transition-all duration-300 border border-gray-100 hover:border-blue-200">
                    ...
                    <div className="flex items-center space-x-3">
                      {/* <div className="flex-shrink-0">
                        <img
                          ...
                          onError={(e) => {
                            e.target.src = "https://via.placeholder.com/64x64/f3f4f6/9ca3af?text=Player";
                          }}
                        />
                      </div> */}
                      <div className="flex-1 min-w-0">
                        <h2 className="text-lg font-bold text-gray-900 mb-1">{player.name}</h2>
                        {player.team && <span className="inline-block mt-1 px-2 py-1 text-xs font-medium bg-blue-100 text-blue-800 rounded-full">{player.team}</span>}
                      </div>
                    </div>
            ...
            <div className="text-center py-12">
              <div className="bg-white rounded-lg shadow-md p-8 max-w-md mx-auto">
                ...
                <p className="text-gray-600 mb-4">Non ci sono giocatori che corrispondono alla ricerca "{searchTerm}".</p>
          ...
          <div className="text-center py-12">
            <div className="bg-white rounded-lg shadow-md p-8 max-w-md mx-auto">
              <h3 className="text-xl font-semibold text-gray-800 mb-2">Nessun giocatore trovato</h3>
              <p className="text-gray-600">Al momento non ci sono giocatori disponibili.</p>
            </div>
          </div>
        )}
      </div>
